...
const TOOLS = [
  {
    key: "adb" as const,
    label: t("Android Platform Tools"),
    sub: t("adb — installed automatically on first launch"),
    cmd: "brew install --cask android-platform-tools",
  },
  {
    key: "scrcpy" as const,
    label: "scrcpy",
    sub: t("screen mirroring & phone camera"),
    // Not translatable: it is a command to paste, not a sentence to read.
      ...
      <div className="rise card-raised float-lg w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="font-display text-[17px] font-semibold text-fg">{t("Set up mirroring tools")}</h2>
      ...
      <div className="flex items-center gap-2">
        ...
        {installed ? (
          <span className="flex items-center gap-1 text-[11px] font-medium text-ok">
            <Icon name="check" size={11} />{t("Installed")}
          </span>
        ) : (
          <span className="text-[11px] font-medium text-bad">{t("Not installed")}</span>
        )}
      </div>
        ...
        <>
          {onInstall && brew && (
            <button onClick={install} disabled={busy} className="btn btn-primary mt-3 w-full">
              ...
              {busy ? t("Installing — this can take a minute…") : t("Install with Homebrew")}
            </button>
          )}
          ...
          <div className="mt-2.5 flex items-center gap-1.5">
            ...
            <button onClick={copy} title={t("Copy command")} className="btn-icon shrink-0">
