...
object WallpaperRepo {
    ...
    fun bytes(ctx: Context): ByteArray {
        val drawable = WallpaperManager.getInstance(ctx).drawable
            ?: throw IllegalStateException("No wallpaper available")

        val source = (drawable as? BitmapDrawable)?.bitmap ?: run {
